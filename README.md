...
| `src/claude-integration.js` | High-level functions for AI agents — takes a site spec, orchestrates automation |
| `mcp/carrd-mcp-server.js` | MCP server — exposes automation as native Claude tools |
| `cli.js` | CLI interface — create sites from terminal with templates |
...
| Form | `form` | Contact, signup, etc. |
| Icon | `icon` | Social/UI icons |
...
- [ ] Record and map Carrd's actual CSS selectors
- [ ] Add visual regression testing with screenshot comparison
- [ ] Support Carrd Pro features (custom domains, multi-page)
- [ ] Add retry logic for flaky UI interactions
- [ ] Build OpenAI Codex and Google Antigravity adapters
